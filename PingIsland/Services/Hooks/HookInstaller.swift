...
struct HookInstaller {
    ...
    static func managedPluginSource(for profile: ManagedHookClientProfile) -> String {
        ...
        const ASSISTANT_DELTA_FLUSH_MS = 750;
        ...
        function boundedText(value, limit = MAX_ASSISTANT_TEXT) {
        ...
        function appendAssistantDelta(session, partID, delta) {
          ...
          const next = boundedText(`${previous}${delta}`);
          ...
          if (type === "message.part.updated" && isObject(properties.part) && stableString(properties.part.sessionID)) {
            ...
            if (properties.part.type === "text" && stableString(properties.part.messageID)) {
              ...
              if (meta.role === "assistant") {
                session.lastAssistantText = boundedText(text);
                if (stableString(properties.part.id)) {
                  session.assistantDeltas.set(stableString(properties.part.id), session.lastAssistantText);
                }
              }
              return undefined;
            }
        ...
        async function runBridge(payload, captureResponse = false) {
          ...
          try {
            ...
            }, captureResponse ? 86_400_000 : 2_000);
        ...
        const pendingAssistantNotifications = new Map();
        const assistantNotificationTimers = new Map();

        function isAssistantNotificationPayload(payload) {
          return payload?.hook_event_name === "Notification"
            && payload?.notification_type === "assistant_message";
        }

        function scheduleAssistantNotification(payload) {
          const sessionId = stableString(payload?.session_id);
          if (!sessionId) return false;

          pendingAssistantNotifications.set(sessionId, payload);
          if (assistantNotificationTimers.has(sessionId)) {
            return true;
          }

          const timer = setTimeout(async () => {
            assistantNotificationTimers.delete(sessionId);
            const latest = pendingAssistantNotifications.get(sessionId);
            pendingAssistantNotifications.delete(sessionId);
            if (latest) {
              await runBridge(latest, false);
            }
          }, ASSISTANT_DELTA_FLUSH_MS);
          assistantNotificationTimers.set(sessionId, timer);
          return true;
        }

        async function flushAssistantNotification(sessionId) {
          const normalizedSessionId = stableString(sessionId);
          if (!normalizedSessionId) return;

          const timer = assistantNotificationTimers.get(normalizedSessionId);
          if (timer) {
            clearTimeout(timer);
            assistantNotificationTimers.delete(normalizedSessionId);
          }
          const latest = pendingAssistantNotifications.get(normalizedSessionId);
          pendingAssistantNotifications.delete(normalizedSessionId);
          if (latest) {
            await runBridge(latest, false);
          }
        }
        ...
        export const server = async ({ client, serverUrl }) => {
          ...
          event: async ({ event }) => {
            ...
            if (isAssistantNotificationPayload(payload)) {
              scheduleAssistantNotification(payload);
              return;
            }

            if (payload.hook_event_name === "Stop" || payload.hook_event_name === "SessionEnd") {
              await flushAssistantNotification(payload.session_id);
            }
