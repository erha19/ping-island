...
struct HookInstaller {
    ...
    static func bridgeHealthStatus() -> BridgeHealthStatus {
        ...
        guard FileManager.default.fileExists(atPath: socketPath) else {
            return BridgeHealthStatus(
                isHealthy: false,
                message: AppLocalization.string("Bridge 监听尚未启动，请保持灵动码正在运行后重试")
            )
        }
        ...
        guard runBridgeLauncherHealthCheck(launcherURL) else {
            return BridgeHealthStatus(
                isHealthy: false,
                message: AppLocalization.string("Bridge launcher 自检失败，请重启灵动码后重新安装 Hooks")
            )
        }
    ...
    private static func preferredBridgeBinaryURL() -> URL? {
        let candidates = [
            Bundle.main.executableURL?
                .deletingLastPathComponent()
                .appendingPathComponent(bridgeBinaryName),
            URL(fileURLWithPath: "/Users/ping-island/PingIsland/Prototype/.build/debug/\(bridgeBinaryName)")
        ]
        .compactMap { $0 }
    ...
    private static func managedMarker(for profile: ManagedHookClientProfile) -> String {
        "NotchCode managed integration: \(profile.id)"
    }
    ...
    static func managedPluginSource(for profile: ManagedHookClientProfile) -> String {
        ...
        return """
        // \(marker)
        // Generated by NotchCode. Reinstall from NotchCode settings if you need to refresh it.
        ...
        export const server = async ({ client, serverUrl }) => {
          ...
          return {
          name: "NotchCode",
          description: "Forward OpenCode hook events to NotchCode.",
          event: async ({ event }) => {
    ...
    static func managedPluginDirectoryFiles(for profile: ManagedHookClientProfile) -> [String: String] {
        ...
        name: ping_island
        version: 1.0.0
        description: Forward Hermes Agent plugin hooks to NotchCode
        provides_hooks:
          - on_session_start
          - pre_llm_call
          - pre_tool_call
            ...
            if (isDangerousBash(event)) {
              ...
              try {
                ...
                if (deniedByIsland(response)) {
                  return { block: true, reason: "Blocked by NotchCode" };
                }
              } finally {
    ...
    static func managedHookDirectoryFiles(
        ...
        ---
        name: \(hookName)
        description: "Forward OpenClaw internal hook events to NotchCode"
        metadata:
        ...
        const handler = async (event) => {
          ...
          try {
            ...
            // OpenClaw hooks should never fail because NotchCode is unavailable.
          }
        };
