...
- **Kimi 专属宠物** - Kimi CLI 默认使用带月光元素和玫红识别色的“Kimi 月亮兔”，让 Kimi hook 会话和其他 hook 客户端区分开来。
...
Kimi CLI 通过官方 hooks 接入，Ping Island 会管理 `~/.kimi/config.toml` 里的 `[[hooks]]` 条目，同时保留 providers、models、loop_control 等非 Island 配置。Kimi 的 `Stop` 表示单轮回复完成，Ping Island 会把它当作回合结束提示；只有 `SessionEnd` 才会真正结束会话。README 顶部的“Kimi 月亮兔”GIF 来自同一套 `MascotView` 导出流程。
