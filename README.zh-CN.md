...
- **Notch 文件投递** - 把文件或图片拖到 Island，即可把本地路径投递给活跃的 Claude、Codex、OpenCode、Qwen、Qoder 兼容会话。
- **统一打扰控制** - Smart Suppress、安静时段、跟随焦点共用一套 attention policy，统一控制自动展开、通知与声音。
- **精确跳转包** - 在 Terminal、iTerm2、Ghostty、tmux 之外，补上 Warp tab 查找、WezTerm/Kaku/Zellij pane 激活和 JetBrains workspace 路由。
- **Hook Health Center** - 在设置里查看 installed、missing、stale、conflict 状态，并可一键重装、卸载、导出诊断。
- **SSH 终端支持** - 可以通过 SSH 自动引导远程 PingIslandBridge，把远程 Claude 兼容 hooks 重写到桥接入口，并把远程终端里的事件统一回流到你本机的 Island 界面。
- **自托管移动中继预览** - 通过本地 relay 服务配对 iOS/watch companion skeleton，接收 session、permission、question 推送。
- **硬件模拟器** - 用 macOS 模拟窗口预览未来 BLE 硬件帧协议中的 mascot、status、tool、亮度和方向。
