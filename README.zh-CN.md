...
<p align="center">
  <img src="docs/images/ping-island-undocked-pet-feature.png" width="960" alt="Ping Island 宠物离岛功能示意图">
</p>

Ping Island 在 `0.4.0` 里正式支持了“宠物离岛”功能：把宠物从刘海里拖出来，变成一个悬浮在桌面上的小助手。

- 它会贴近当前激活窗口显示，而不是一直占用刘海区域。
- 你可以通过 hover 或点击宠物，打开锚定式气泡来处理审批、追问和状态查看。
- Ping Island 会记住宠物位置，并支持通过右键宠物重新打开设置面板。
...
- **宠物离岛** - 可把宠物从刘海里拖出来变成桌面悬浮宠物，并通过锚定式气泡继续处理活跃会话。
...
本 README 里的宠物 GIF 来自真实 `MascotView` 实现，通过 `./scripts/render-mascots.sh` 生成；“宠物离岛”示意图资源 `docs/images/ping-island-undocked-pet-feature.png` 通过 `./scripts/render-detached-pet-poster.sh` 生成。
