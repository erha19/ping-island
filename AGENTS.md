...
- If you add a Claude-compatible hook client, start in `PingIsland/Models/ClientProfile.swift` and wire any truly client-specific behavior from there before adding new ad-hoc switches elsewhere.
  ...
  - Qoder App is a separate runtime profile (`qoder-app`, `com.qoder.app`); Qoder CN uses the product name `Qoder CN`, with `qoder-cn-app` reserved for explicit `parent_business_info.product=app` evidence because its bundle overlaps the IDE. Real source processes and payload product beat shared hook arguments. App questions retain the blocking answer channel, automatically submit available default options, and keep the QoderWork-style question notice / open-client action; ordinary App approvals remain actionable. Shared desktop hooks need the same long question/approval timeout as CLI hooks. See [Qoder identity and question routing](docs/qoder-clients.md).
...
- If you change notch sizing, opening behavior, or visibility, inspect both `NotchViewModel` and `NotchView`.
  ...
  - Approval attention deduplication must use the actual request key, including intervention IDs when no `activePermission` exists. Consecutive approvals in one session need independent notifications and independent auto-approval delays.
