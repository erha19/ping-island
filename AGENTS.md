...
- Terminal and focus control: `PingIsland/Services/Tmux/`, `PingIsland/Services/Window/`, `PingIsland/Utilities/TerminalVisibilityDetector.swift`
  ...
  - `suppressWhenSessionHostFocused` is a separate opt-in setting from `smartSuppression`, and defaults off. `smartSuppression` asks only whether a terminal window is visible somewhere on the space and governs the pending-session auto-open; do not extend it to mean host focus, since that is a different question with a wider blast radius. The new setting gates attention-driven presentation, both completion-notification queues (docked and detached), and notification sounds: when the session's own host app is frontmost, those stay where the user is already looking. Resolve the host through `TerminalVisibilityDetector.isSessionHostFocused`, and check the setting first so no host resolution happens while it is off.
  - `isSessionHostFocused` must not depend on a session pid. Agents running as an IDE extension report no pid in their hook metadata, so pid-only checks are unreachable for exactly those sessions; match `SessionClientInfo.hostBundleIdentifier` against the frontmost app instead, folding helper bundles onto their host. That property reads `terminalBundleIdentifier` first and falls back to `bundleIdentifier`, which is what app-server clients record; never use `terminalProgram`, which can hold a stale value inherited from the launching shell. Only bundles the registry recognises count as hosts, which also keeps the Island's own bundle from suppressing its approvals.
  - Process resolution stays as the fallback when a pid exists, and decides ownership by looking for the frontmost application's pid in the session's parent chain (`ProcessTreeBuilder.isAncestor`). Do not resolve the host by process name there: an IDE-hosted agent binary can live inside the IDE's own extension directory, so `TerminalAppRegistry.isTerminal` matches the agent and the IDE helper processes ahead of the application pid that `NSWorkspace` reports as frontmost.
  - Both notification-sound call sites share this resolution. A missing pid must never be read as "not focused", or sounds fire for sessions the user is watching.
  - Suppression applies only to prompts that exist somewhere else. With `routePromptsToTerminal` off the bridge blocks and the Island holds the only copy of an approval, so `SessionState.islandOwnsBlockingPrompt` must veto suppression there — withholding it would stall the agent behind a closed-notch indicator. Completion panels and sounds carry no such risk and stay unconditional.
  - Host matching is bundle-level, so two windows of the same IDE are indistinguishable (one process, one bundle). A session in another window of a frontmost IDE counts as focused; the ownership veto keeps that from ever hiding a blocking approval.
