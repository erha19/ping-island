...
- `PingIsland` is a macOS menu bar app that surfaces Dynamic Island-style status for Claude Code, Codex, Gemini CLI, Antigravity CLI, Hermes Agent, Qwen Code, Kimi CLI, Kiro, and compatible hook-driven agent sessions.
...
- If you add a Claude-compatible hook client, start in `PingIsland/Models/ClientProfile.swift` and wire any truly client-specific behavior from there before adding new ad-hoc switches elsewhere.
  ...
  - Kiro hooks are managed as a dedicated `~/.kiro/hooks/ping-island.json` v1 file. Keep its actions observational (`SessionStart`, `UserPromptSubmit`, `PostToolUse`, and `Stop`) so Ping Island reports activity without blocking Kiro tools or taking over its permission flow.
